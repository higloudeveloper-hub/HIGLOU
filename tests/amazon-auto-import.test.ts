...
describe("Amazon auto-import stays an eBay draft flow", () => {
  it("lets the seller pick a category, count, and which winners to import", () => {
    ...
    expect(winnersStudio).toMatch(/FindWinnersBoard/);
    expect(winnersStudio).toMatch(/asins: next/);
    expect(winnersStudio).toMatch(/mode: importMode/);
    expect(winnersStudio).not.toMatch(/AmazonAutoImportPanel/);
    ...
    expect(importRoute).not.toMatch(/fast: true/);
    ...
    expect(amazonImport).toMatch(/importAmazonCatalogProduct/);
    expect(workspace).toMatch(/analyzing for eBay/);
